...
from reportlab.platypus import SimpleDocTemplate, Table, TableStyle, Paragraph, Spacer, HRFlowable, Image, PageBreak, CondPageBreak
...
def _parse_inv_date(s):
    """Parsira datum računa u (god, mj, dan) tuple za sort.
    Podržava: YYYY-MM-DD, DD.MM.YYYY, D.M.YYYY i sve varijante."""
    import re as _re
    if not s:
        return (0, 0, 0)
...
def invoice_list():
    ...
    invoices = sorted(
        invoices_raw,
        key=lambda r: (_parse_inv_date(r['invoice_date']), r['id']),
        reverse=True
    )
...
def leave_list():
    ...
    employee_quotas = []
    if is_approver:
        quota_employees = employees if (user.get('is_admin') or user.get('can_approve_leave')) else             [e for e in employees if e['id'] in (managed_employee_ids + ([current_employee_id] if current_employee_id else []))]
